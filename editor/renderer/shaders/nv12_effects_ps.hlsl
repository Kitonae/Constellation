...
float4 PSMain(float4 pos : SV_Position, float2 uv : TEXCOORD0) : SV_Target {
    ...
    // 1088-line surface. uvScale trims the padding the hardware wrote past the
    // visible picture, and uvMax keeps the far edge a half texel inside it so
    // the bilinear tap cannot reach the first padding row. Both are (1,1) for
    // decoders that hand back an exactly-sized texture.
    float2 suv = min(uv * uvScale, uvMax);
